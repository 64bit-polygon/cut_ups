		...
		funcs: {
			...
			launchShowCloudAnimation: function() {
				...
				*/
				...
				var $cloudGroup = SVG.find( '#cloudGroup' );
				
				F.changeSplashFeatureSlide();
				
				$cloudGroup.animate( 350, 0, 'now' ).y( 70 ).opacity( 1 ).ease('>').after( function() {
					$cloudGroup.animate( 350, 3000, 'now' ).y( 100 ).opacity( 0 ).ease('>');
					
					setTimeout(
						function() {
							F.changeSplashFeatureSlide();
							SVG.find( '#splashBlinkingCursor' ).attr( 'class', 'cursor visible animating' );
						},
					5000 );
				});
			},
			...
			launchSplashTextMergeAnimation: function() {
				...
				*/
				...
				var $cLeftText = SVG.find( '#centerPageImg .text.left' ),
						$cRightText = SVG.find( '#centerPageImg .text.right' ),
						dur = 400,
						delayBase = dur / 4,
						i = 0,
						$lText = SVG.find( '#leftPageImg .text' ),
						l = $lText.length,
						$rText = SVG.find( '#rightPageImg .text' ),
						cL = $cLeftText.length;
				
				for ( i; i < l; i = i + 1 ) {
					$lText[ i ].animate( dur, delayBase * i, 'now' ).x( 100 ).ease( '>' );
					$rText[ i ].animate( dur, delayBase * i, 'now' ).x( -100 ).ease( '>' );
					
					if ( i < cL ) {
						$cLeftText[ i ].animate( dur, delayBase * i, 'now' ).x( 118 ).ease( '>' );
						$cRightText[ i ].animate( dur, delayBase * i, 'now' ).x( 118 ).ease( '>' );
					}
				}
			...
			launchSplashTextTumbleAnimation: function() {
				...
				*/
				...
				var $blinkingCursor = SVG.find( '#splashBlinkingCursor' ),
						cnt = 0,
						cx,
						i = 0,
						dur = 1000,
						$texts = SVG.find( '#centerPageImg .initTextGroup .text' ),
						l = $texts.length,
						$text,
						w,
						y;
				...
				for ( i; i < l; i = i + 1 ) {
					...
					y = $text.y() + w / 2 + 200;
					$text.animate( dur, dur, 'now' ).attr( 'y', y ).rotate( deg, cx, 0 );
				}
				
				setTimeout(
					function() {
						$blinkingCursor.attr( 'class', 'cursor visible animating' );
					},
				dur );
				
				setTimeout(
					function() {
						$blinkingCursor.attr( 'class', 'cursor notVisible notAnimating' );
			...
			launchSplashTextWhiteOutAnimation: function() {
				...
				*/
				...
				function animateWhiteOut() {
					$cursor = SVG.find( '#whiteOutGroup .cursor' )[ i ];
					$whiteOut = SVG.find( '#whiteOutGroup .whiteOut' )[ i ];
					$cursor.opacity( 1 );
					$whiteOut.opacity( 1 );
					$whiteOut.animate( dur, delay, 'now' ).plot( allWhiteOutCoords[ i ] ).ease('>');
					$cursor.animate( dur, delay, 'now' ).x( cursorCoords[ i ] ).ease('>').after( function() {
						$cursor.opacity( 0 );
						i = i + 1;
						if ( i < l ) {
							animateWhiteOut();
							...
							setTimeout(
								function() {
									$baseTextGroup = SVG.find( '#baseTextGroup' );
									$whiteOutGroup = SVG.find( '#whiteOutGroup' );
									$baseTextGroup.animate( dur, delay, 'now' ).y( 35 ).opacity( 0 ).ease("=");
									$whiteOutGroup.animate( dur, delay, 'now' ).y( 35 );
								},
							1000 );
			...
			launchSplashTextWriteAnimation: function() {
				...
				*/
				...
				var $cursor,
						$writeTextSvgs = SVG.find( '.writeText' ),
				...
				for ( i; i < l; i = i + 1 ) {
					$writeTextSvg = $writeTextSvgs[ i ];
					$writeTextElem = $writeTexts[ i ];
					$cursor = SVG.find( '.cursor', $writeTextElem);
					x = i < 2 ? 185 : 208;
					dur = 2 * x;
					$cursor.animate( 0.1, i * dur + 200, 'now' ).fill( '#000' ).ease("=");
					$writeTextSvg.animate( dur, i * dur + 201, 'now' ).x(x).ease("=");
					$writeTextSvg.animate( 100, i * dur + 201 + dur, 'now' ).opacity( 0 );
				}
			...
			setUpSplashFeaturesAnimation: function() {
				...
				*/
				...
				L.draw = SVG().addTo('#leftPageImg').size( 90, 120 );
				L.draw.svg(lSvg);
				L.$wrap = $leftPage;
				
				C.draw = SVG().addTo('#centerPageImg').size( 326, 200 );
				C.draw.svg( mainSvg );
				C.$wrap = $page;
				
				R.draw = SVG().addTo('#rightPageImg').size( 90, 120 );
				R.draw.svg( rSvg );
				R.$wrap = $rightPage;
								
				SVG.find('#cloudGroup, #whiteOutGroup > *').each( function() {
					this.opacity( 0 );
				});

				setTimeout(
					function() {
						$( '#leftPageImg' ).removeClass( 'notVisible' );
						$( '#centerPageImg' ).removeClass( 'notVisible' );
						$( '#rightPageImg' ).removeClass( 'notVisible' );
					},
				500 );
